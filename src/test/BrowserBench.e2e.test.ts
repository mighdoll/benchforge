...
test("bench function mode (window.__bench)", { timeout: 30000 }, async () => {
  ...
  // gcEvents now carries only full (mark-compact) GCs; the high-frequency
  // scavenge/minor-ms events are dropped, and their count lives in gcStats. The
  // allocation-heavy example reliably triggers GC (scavenges at least), and the
  // retained events carry loop-relative offsets rebased from the trace clock via
  // the loop-start mark.
  const { scavenges, markCompacts } = result.gcStats!;
  expect(scavenges + markCompacts).toBeGreaterThan(0);
  expect(result.gcEvents?.length ?? 0).toBe(markCompacts);
