...
test("live viewer: report tab shows chart SVG", { timeout: 30_000 }, async () => {
...
test("live viewer: allocation tab has speedscope content", { timeout: 30_000 }, async () => {
  const browser = await chromium.launch();
  try {
    const page = await browser.newPage();
    await page.goto(`http://localhost:${port}`, { waitUntil: "networkidle" });
...
test("live viewer: timing tab has speedscope content", { timeout: 30_000 }, async () => {
  const browser = await chromium.launch();
  try {
    const page = await browser.newPage();
    await page.goto(`http://localhost:${port}`, { waitUntil: "networkidle" });
